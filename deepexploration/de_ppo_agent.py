...
class PPOAgent(BaseAgent):
    ...
    def __post_init__(self):
        ...
        for critic in self.critics: 
            critics_params.extend(list(critic.parameters()))

        all_params = list(self.actor.parameters()) + critics_params
        ...
        if self.same_body:
            ...
            critics_param_dicts = []
            for critic in self.critics: 
                critics_param_dicts.append({'params': critic.parameters(), 
                                            'lr': cfg.alg.value_lr}) 
            all_params_list = [{'params': self.actor.parameters(),
                                      'lr': cfg.alg.policy_lr}]
            all_params_list.extend(critics_param_dicts)
            optim_args['params'] = all_params_list
    ...
    def get_act_val(self, ob, *args, **kwargs):
        """
        instead of returning a singular value per ob, it will return 
        num_critics values per ob
        """
        ob = torch_float(ob, device=cfg.alg.device)
        act_dist, body_out = self.actor(ob)
        vals = []
        if self.same_body:
            print("trouble trouble, abort bc actor and critic share body??")
            val, body_out = self.critic(body_x=body_out)
        else:
            for critic in self.critics:
                val, body_out = critic(x=ob)
                val = val.squeeze(-1)
                vals.append(val)
            
        #TODO should vals be a tensor or a list?          
        return act_dist, vals

    @torch.no_grad()
    def get_val(self, ob, *args, **kwargs):
        # must be k-dimensional to be consistent with val 
        self.eval_mode()
        ob = torch_float(ob, device=cfg.alg.device)
        val, body_out = self.critic(x=ob)
        val = val.squeeze(-1)
